...
def suggest_design(
    ...
    """Suggest experimental design(s) based on factors, phase, model order, budget, and constraints.
    
    Parameters
    ----------
    factors : dict
        
    phase : {'screening', 'optimization'}, default='optimization'
          
    model_order : {'linear', '2FI', 'quadratic'}, optional
    ...
    Notes
    -----
    For screening phase with continuous factors, they should be configured with 
    2 levels (low/high). For optimization, use 3+ levels.
    """
...
def _optimization_process_only(
    ...
    if performed_exp:
            ...
            recommendations.append(DesignRecommendation(
                design_name="D-Optimal Augmentation",
                n_runs= "Flexible, up to budget",
                pros=[
                    "Adds new runs to maximize information gain given the existing data",
                    "Respects feasibility constraints (avoids invalid factor combinations)",
        ...
        if non_rectangular_constraints:
            ...
            # Pure continuous factors -> RSM suggestions
                ...
                if k == 2:
                    ...
                    recommendations.append(DesignRecommendation(
                        design_name="Full Factorial Design (3-level)",
                        n_runs=3**k,
                        pros=[
                            "Simple, structured design with good coverage of the factor space",
                            "Includes center and mid-level points to capture curvature",
                            f"Fits within budget ({3**k} ≤ {max_experiments} runs)"],
                        cons=[
                            "Run count grows quickly with the number of factors (3^k)",
                            "Can consume a large fraction of the budget, leaving limited room for replication or follow-up"
                        ],
                        use_case=[
                            "Optimization when a full 3-level factorial is feasible and you want comprehensive coverage of \n"
                            "the factor space to fit a quadratic model without relying on a specific RSM structure"
                        ]))
                        
                    recommendations.append(DesignRecommendation(
                        design_name="Central Composite Design (CCD)",
                        n_runs=(2 ** k) + (2 * k) + DEFAULT_CENTER_POINTS,
                        pros=[
                ...
                if k >= 3:
                    ...
                    if max_experiments is None or (2 * k * (k - 1)) + DEFAULT_CENTER_POINTS <= max_experiments:
                        recommendations.append(DesignRecommendation(
                            design_name="Box–Behnken Design (BBD)",
                            n_runs=(2 * k * (k - 1)) + DEFAULT_CENTER_POINTS,
                            pros=[
                                "Efficient for fitting second-order (quadratic) response surface models",
                    ...
                    if max_experiments is None or (2**k) + (2 * k) + DEFAULT_CENTER_POINTS <= max_experiments:
                        recommendations.append(DesignRecommendation(
                            design_name="Central Composite Design (CCD)",
                            n_runs=(2 ** k) + (2 * k) + DEFAULT_CENTER_POINTS,
                            pros=[
                    ...
                    recommendations.append(DesignRecommendation(
                        design_name="D-Optimal Design for Process Optimization",
                        n_runs= "Flexible, up to budget",
                        pros=[
                            "Efficient initial run allocation targeted to the specified model under a fixed budget",
                            "Works well in constrained or irregular domains where classical RSM designs are infeasible",
                            "Supports custom model structures (e.g., include/exclude specific interactions or higher-order terms)"
                        ],
                        cons=[
                            "Requires specification of the desired model structure",
                            "May be computationally intensive for large-scale problems"
                        ],
                        use_case=["Optimization when the experimental region is constrained or irregular and classical designs are infeasible"],
                        additional_info=[
                            "D-optimal designs are particularly useful when the goal is to estimate a specific set of model parameters with maximum precision.",
                            "They can be tailored to accommodate unique constraints or requirements of the experimental setting."
                        ]
                    ))
